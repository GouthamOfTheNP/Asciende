...
if "asl_text" not in st.session_state:
	st.session_state.asl_text = ""  # store detected ASL text
...
if run:
	while True:
		...
		# Replace this with your ASL model output later
		st.session_state.asl_text = st.text_input(
			"ASL Output (for testing)", st.session_state.asl_text
		)

		# Show Play button only if ASL text is detected
		if st.session_state.asl_text.strip():
